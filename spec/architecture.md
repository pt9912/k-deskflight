...
| `AR-OP-005` | Anwender-Overridebarkeit des Default-Operator-Namespace `k-deskflight-system` via Kustomize-Overlay (ab MVP) bzw. Helm-Values (ab v0.2, `ADR 0005`) — exakte Override-Mechanik | Kustomize-Base liegt in `deploy/manifests/kustomization.yaml` (vorgesehen für `kubectl apply -k`); die Cluster-Smoke-Pipeline (`scripts/cluster-smoke.sh`) nutzt das Pattern operativ. **Explizite User-Doku zur Namespace-Override-Mechanik** entsteht in M6 (Anwender-Doku, `docs/user/`); bleibt bis dahin offen. |
