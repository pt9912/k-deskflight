...
| Item | Datum | Notiz |
| ---- | ----- | ----- |
| §7 #9 — `LH-AK-005` Cluster-Smoke gegen reale Server-Version | 2026-05-17 | Logik via fake-API + Reconciler-Tests im Slice attestiert; zusätzlich durch ersten `cluster-smoke`-CI-Lauf (ADR 0013) **real verifiziert**: `discovery.ServerVersion()` gegen kindest/node:v1.34.0 liefert `v1.34.0`, Reconciler schreibt `status.phase: Passed` mit Condition `KubernetesVersionReady=True` und Message `"server version 1.34.0 satisfies minimum 1.34"`. Status-Dump im Run-Artefakt `cluster-smoke-attest`. Run-URL: <https://github.com/pt9912/k-deskflight/actions/runs/25999750149>. Der discovery-Adapter wird damit auch real abgedeckt — envtest-Backfill bleibt eine M6-Add-Möglichkeit, ist aber nicht mehr blockierend. |
